...
gpio_pin_enum            ips_rst_pin             = IPS200_RST_PIN_SPI;
gpio_pin_enum            ips_bl_pin              = IPS200_BLk_PIN_SPI;
...
////绘制点，增加点半径参数
//void ips200_draw_point_plus(uint16 x, uint16 y, const uint16 color,uint8 radius)
//{
//    // 如果程序在输出了断言信息 并且提示出错位置在这里
//    // 那么一般是屏幕显示的时候超过屏幕分辨率范围了
//	zf_assert(x < ips200_x_max);
//  zf_assert(y < ips200_y_max);
//	int i=0,j=0,half_size=0;
//	half_size=radius/2;
//	for(i=-half_size;i<half_size;i++)
//	{	
//		for(j=-half_size;j<half_size;j++)
//			ips200_draw_point(clip(x+i,0,ips200_x_max-1),clip(y+j,0,ips200_y_max-1),color);
//	}
...
//              如果要显示二值化图像 直接修改最后一个参数为需要的二值化阈值即可
//-------------------------------------------------------------------------------------------------------------------
//修改了图像参数，将图像参数修改为二维指针，用于显示透视变换后图像
void ips200_show_gray_image (uint16 x, uint16 y, const uint8 *image, uint16 width, uint16 height, uint16 dis_width, uint16 dis_height, uint8 threshold)
{
    ...
    for(j = 0; j < dis_height; j ++)
    {
        ...
        for(i = 0; i < dis_width; i ++)
        {
            width_index = i * width / dis_width;//此处有修改
...
//显示逆透视变换后的图像
void ips200_show_PTR_image (uint16 x, uint16 y, const uint8 **image, uint16 width, uint16 height, uint16 dis_width, uint16 dis_height, uint8 threshold)
{
    // 如果程序在输出了断言信息 并且提示出错位置在这里
    // 那么一般是屏幕显示的时候超过屏幕分辨率范围了
    zf_assert(x < ips200_x_max);
