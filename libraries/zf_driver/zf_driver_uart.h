...
uint8   uart_query_byte                     (uart_index_enum uartn, uint8 *dat); //读取串口接收的数据（查询接收）
...
#define uart_query(uart_n, dat)             (uart_query_byte((uart_n), (dat)))   //query 质疑

#define uart_tx_irq(uart_n, status)         (uart_tx_interrupt((uart_n), (status)))  // tx中断
#define uart_rx_irq(uart_n, status)         (uart_rx_interrupt((uart_n), (status)))  // rx中断
#endif
